 ...
 * 6) 읽기는 웹앱을 거치지 않는다 (호출마다 2~3초라 너무 느리다).
 *    파일 → 공유 → 웹에 게시 → "문의게시판" 시트를 "쉼표로 구분된 값(.csv)"으로 게시하고,
 *    그 주소를 assets/app.js의 BOARD_CSV_URL 에 넣는다. 게시 CSV는 몇 분 늦게 갱신되므로
 *    방금 쓴 글은 사이트가 브라우저에 잠시 기억해 끼워 넣는다.
 * 7) 스프레드시트 시간대(파일 → 설정 → 일반)를 반드시 (GMT+09:00) Seoul 로 둔다.
 *    이 스크립트가 적는 시각은 시트 시간대를 따르고, 사이트는 CSV의 시각을 한국 시간으로
 *    고정 해석한다. 기본값(미국 태평양)이면 16시간 어긋난다.
 *
 * ── 답변 다는 법 ──
